...
def preprocess(img):
    ...
    img = cv2.cvtColor(img, cv2.COLOR_RGB2HSV) #RGB 이미지의 빨간색, 녹색, 파란색 값을 HSV 이미지의 색상, 채도, 명도(HSV) 값으로 변환합니다.
    img = cv2.GaussianBlur(img, (9, 9), 0) #가우시안 필터링 함수
    img = img/255
...
def plotImage(img):
    ...
    plt.show() #이미지를 그래프에다 정형화해서 표현하기
...
def cropOrig(bRect, oimg):
    ...
    y2 = int(h1/10) #높이 나누기 10

    x2 = int(w1/10) #너비 나누기 10
